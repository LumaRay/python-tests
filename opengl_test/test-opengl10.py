...
class Canvas(GLCanvas):
    ...
    def onDraw(self):
        ...
        draw_start = round(time.monotonic() * 1000)
        ...
        # buf_mem_pointer = gl.glMapBuffer(gl.GL_ARRAY_BUFFER, gl.GL_READ_WRITE)  #  gl.GL_WRITE_ONLY)
        ...
        time_taken = round(time.monotonic() * 1000) - draw_start
        global test_cnt, test_acc
        test_cnt += 1
        test_acc += time_taken
        print("draw time", time_taken, "action_time_taken", action_time_taken, "ms, acc =", round(test_acc / test_cnt), "ms")
