...
from tensorflow.keras.layers import ConvLSTM2D, Activation, MaxPooling3D, Dense, Dropout, Reshape, LSTM, Flatten, BatchNormalization, Input, concatenate
...
def createInputModel(input_shape):
    ...
    x = Dense(64*32, activation="sigmoid")(x)
    #x = Dropout(0.1)(x)
    x = Dense(64*8, activation="sigmoid")(x)
    model = Model(model_input, x)
    return model_input, model

input_1, model_1 = createInputModel((1, face_width, face_height))
input_2, model_2 = createInputModel((1, face_width, face_height))
input_3, model_3 = createInputModel((1, face_width, face_height))
input_4, model_4 = createInputModel((1, face_width, face_height))
input_5, model_5 = createInputModel((1, face_width, face_height))
input_6, model_6 = createInputModel((1, face_width, face_height))

combined = concatenate([model_1.output, model_2.output, model_3.output, model_4.output, model_5.output, model_6.output])
x = Flatten()(combined)
x = Dense(4*64 * 64, activation="sigmoid")(x)
#x = Dropout(0.1)(x)
...
model = Model(inputs=[model_1.input, model_2.input, model_3.input, model_4.input, model_5.input, model_6.input], outputs=x)
...
x_train = np.zeros((TRAIN_FACE_SEQUENCES_COUNT, FACE_SEQUENCE_SIZE, face_height, face_width, 6), dtype=np.float64)
y_train = np.zeros((TRAIN_FACE_SEQUENCES_COUNT, 1, 1), dtype=np.float64)

x_val = np.zeros((VALID_FACE_SEQUENCES_COUNT, FACE_SEQUENCE_SIZE, face_height, face_width, 6), dtype=np.float64)
y_val = np.zeros((VALID_FACE_SEQUENCES_COUNT, 1, 1), dtype=np.float64)

x_test = np.zeros((TEST_FACE_SEQUENCES_COUNT, FACE_SEQUENCE_SIZE, face_height, face_width, 6), dtype=np.float64)
y_test = np.zeros((TEST_FACE_SEQUENCES_COUNT, 1, 1), dtype=np.float64)
...
for file_color in list_dir:
    if file_color.endswith(".jpg"):
        ...
        try:
            ...
            frame_color = (frame_color / 256).astype(np.float64)
            ...
            frame_depth = np.expand_dims((frame_depth / 65536).astype(np.float64), axis=2)
            ...
            #combined_frame = combined_frame.astype(np.float64) / 255
        ...
        if len(list_faces) == FACE_SEQUENCE_SIZE:
            np_combined_frames = np.zeros((FACE_SEQUENCE_SIZE, face_height, face_width, 6), dtype=np.float64)
            np_temps = np.zeros((FACE_SEQUENCE_SIZE, 1), dtype=np.float64)
...
history = model.fit(
    x=list(np.moveaxis(x_train, -1, 0)),
    y=y_train,
...
results = model.evaluate(list(np.moveaxis(x_test, -1, 0)), y_test, batch_size=1)#128)
