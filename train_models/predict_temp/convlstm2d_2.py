...
VALID_FACE_SEQUENCES_COUNT = 5
TEST_FACE_SEQUENCES_COUNT = 5
...
for color_file in color_files:
    ...
    if len(color_faces_list) > FACE_SEQUENCE_SIZE:
        ...
        if face_sequence_idx < TRAIN_FACE_SEQUENCES_COUNT:
            ...
            x_val[val_set_idx] = combined_frames
            y_val[val_set_idx, 0] = temps[-1] / 100
            val_set_idx += 1
        elif face_sequence_idx < TRAIN_FACE_SEQUENCES_COUNT + VALID_FACE_SEQUENCES_COUNT + TEST_FACE_SEQUENCES_COUNT:
            x_test[test_set_idx] = combined_frames
            y_test[test_set_idx, 0] = temps[-1] / 100
            test_set_idx += 1
        else:
            break
        face_sequence_idx += 1
