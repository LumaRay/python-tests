...
# yolactInputShapeArray = np.array(
#     [TORCH_YOLACT_SHAPE[1], TORCH_YOLACT_SHAPE[0],
#      TORCH_YOLACT_SHAPE[1], TORCH_YOLACT_SHAPE[0]], dtype=np.uint16)

TORCH_UEM_MASK_SHAPE = (128, 128)
TORCH_DETECT_FEATURES_SHAPE = (160, 160)
...
FACE_FEATURES_BASE = cp.zeros((FACE_FEATURES_BASE_SIZE, FACE_FEATURES_COUNT), dtype=cp.float32)
cp_diff_holder = cp.zeros_like(FACE_FEATURES_BASE)
...
with torch.no_grad():
    ...
    A = imp.load_source('MainModel', '/home/thermalview/Desktop/ThermalView/tests/keras_torch/keras_to_torch_uem_mask_2.py')
...
# cp_stream = cp.cuda.Stream(non_blocking=False)
...
while True:
    ...
    dcv_frame_bgr = dcv_frame

    # d_frame = cv2.cuda.cvtColor(d_frame, cv2.COLOR_BGR2GRAY)
    # d_frame = cv2.cuda.cvtColor(d_frame, cv2.COLOR_GRAY2BGR)

    # d_frame = cv2.cuda.cvtColor(d_frame, cv2.COLOR_BGRA2GRAY)
    # d_frame = cv2.cuda.cvtColor(d_frame, cv2.COLOR_GRAY2BGR)
    # d_frame = d_frame.clone()
    ...
    # cp_frame[0:200, 0:200, 0] = 100
    # cp_arr = cp_arr.reshape((cap_height * cap_width * 4,))
    # cp_arr = cp.ndarray(shape=(cap_height * cap_width * 4,), dtype=cp.uint8, memptr=memptr)
    # cp_arr[0:12340] = 55
    time_taken_cv_to_cp = round(time.monotonic() * 1000) - start_time

    start_time = round(time.monotonic() * 1000)
    cp_frame_gray = cp.dot(cp_frame[..., :3], cp.asarray([0.2989, 0.5870, 0.1140], dtype=cp.float32)).astype(cp.uint8)
    cp_frame_gray = cp.repeat(cp_frame_gray[:, :, cp.newaxis], 3, axis=2)

    # np_frame = cp.asnumpy(cp_arr)

    # np_frame = cp.asnumpy(cp_arr_3)
    ...
    with torch.no_grad():
        ...
        if num_dets_to_consider > 0:
            ...
            for j in reversed(range(num_dets_to_consider)):
                ...
                face_data = FaceData()
                face_data.bbox_relative = np.clip(np.array([xRelWide, yRelWide, wRelWide, hRelWide], dtype=np.float32), 0, 1)
    ...
    for face_data in frame_faces_list:
        ...
        x, y, w, h = (np.array(face_data.bbox_relative) * np.array([cap_width, cap_height, cap_width, cap_height])).astype(np.int32)
    ...
    start_time = round(time.monotonic() * 1000)
    dt_faces_128 = None  # torch.empty((TORCH_UEM_MASK_SHAPE[0], TORCH_UEM_MASK_SHAPE[1]), dtype=torch.float32, device='cuda')
    dt_faces_160 = None
    for face_data in frame_faces_list:
        xRelWide, yRelWide, wRelWide, hRelWide = face_data.bbox_relative
        x, y, w, h = (np.array(face_data.bbox_relative) * np.array([cap_width, cap_height, cap_width, cap_height])).astype(np.int32)
        ...
        dt_face_128 = torch.nn.functional.interpolate(dt_face, TORCH_UEM_MASK_SHAPE)
        dt_face_128 /= 255
        dt_faces_128 = dt_face_128 if dt_faces_128 is None else torch.cat((dt_faces_128, dt_face_128), dim=0)
        dt_face_160 = torch.nn.functional.interpolate(dt_face, TORCH_DETECT_FEATURES_SHAPE)
        dt_face_160 /= 255
    ...
    time_taken_masks_inference = 0
    time_taken_face_features_inference = 0
    time_taken_recognize_faces = 0
    if dt_faces_128 is not None:
        with torch.no_grad():
            # Detect masks
            start_time = round(time.monotonic() * 1000)
        ...
        cp_face_features_base = FACE_FEATURES_BASE
    ...
    '''for face_data in frame_faces_list:
        ...
        x, y, w, h = (np.array(face_data.bbox_relative) * np.array([cap_width, cap_height, cap_width, cap_height])).astype(np.int32)
        # np.full((2, 2), [1, 2])
        # np_text_placeholder = np.array((100, 100, 3), dtype=np.uint8)
        ...
        np_text_placeholder = np.zeros((text_size[0][1], text_size[0][0], 3), dtype=np.uint8)
        cv2.putText(np_text_placeholder,
                    text,
                    (0, np_text_placeholder.shape[0] - 1),
                    ...
                    1,  # thickness
                    cv2.LINE_AA)
        # cp_text_placeholder = cp.asarray(np_text_placeholder)
        dcv_text_placeholder = cv2.cuda_GpuMat()
        dcv_text_placeholder.upload(np_text_placeholder)  # time consuming! 10-30ms
        # https://docs.opencv.org/4.5.3/d3/df2/tutorial_py_basic_ops.html#:%7E:text=making%20borders%20for%20images%20(padding)
        # constant = cv2.copyMakeBorder(cp_text_placeholder, 0, 10, 0, 10, cv.BORDER_CONSTANT, value=BLUE)
        # sz = dcv_frame.size()
        # sz1 = dcv_text_placeholder.size()
        dcv_text_placeholder_large = cv2.cuda.copyMakeBorder(dcv_text_placeholder, 0,
                                dcv_frame.size()[1] - dcv_text_placeholder.size()[1], 0,
                                dcv_frame.size()[0] - dcv_text_placeholder.size()[0], cv2.BORDER_CONSTANT, value=(0, 0, 0))
        # sz2 = dcv_text_placeholder_large.size()
        dcv_text_placeholder_large = cv2.cuda.cvtColor(dcv_text_placeholder_large, cv2.COLOR_RGB2RGBA)
        dcv_frame = cv2.cuda.addWeighted(dcv_frame, 1, dcv_text_placeholder_large, 1, 0.0)
        ...
        # cp_frame[y:y+2, x:x+w, :3] = cp.array([0, 255, 0])
        # cp_frame[y+h:y+h+2, x:x+w, :3] = cp.array([0, 255, 0])'''
    time_taken_draw_text_cpu_gpu = round(time.monotonic() * 1000) - start_time
