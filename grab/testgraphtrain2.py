...
def createImageInputModel(input_shape):
    ...
    x = Activation('relu')(x)
    x = AveragePooling3D(pool_size=(1, 2, 2), data_format='channels_first')(x)
    x = ConvLSTM2D(64, kernel_size=(5, 5), padding='valid', return_sequences=True, data_format='channels_first')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = AveragePooling3D(pool_size=(1, 2, 2), data_format='channels_first')(x)
    ...
    while (x.shape[-1] > 1 or x.shape[-2] > 1):
        # x = MaxPooling3D(pool_size=(1, 2, 2), data_format='channels_first', strides=(1, 2, 2), padding="same")(x)
        x = ConvLSTM2D(nfeatures, kernel_size=(3, 3), padding='same', return_sequences=True, data_format='channels_first')(x)
        nfeatures += 32
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = AveragePooling3D(pool_size=(1, 2, 2), data_format='channels_first', padding="same")(x)
...
for idx_start in range(total_samples):
    ...
    y_samples[idx_start] = cv2.resize(lst_chart[idx_start + HISTORICAL_FRAMES + 1], (res_width, res_height), cv2.INTER_CUBIC)
